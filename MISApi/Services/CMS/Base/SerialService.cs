...
namespace MISApi.Services.CMS.Base
{
    ...
    /// 
    /// </summary>
    public class SerialService : BaseService.EF<Serial, PandoraContext>
    {
        ...
        /// CreateService
        /// </summary>
        public class CreateService : SerialService
        {
            /// <summary>
            /// 定义事务服务
            /// </summary>
            ...
            /// <returns></returns>
            public Serial Execute(Serial entity)
            {
                try
                {
                    // 定义
                ...
                {
                    throw new Exception("MISApi.Services.CMS.Base.SerialService.CreateService.Execute", ex);
                }
            }
            /// <summary>
            /// 默认的事务方法
            ...
            /// <returns></returns>
            public List<Serial> Execute(List<Serial> entityList)
            {
                try
                {
                    // 定义
                    List<Serial> resultList = new List<Serial>();
                    // 事务
                    transService.TransRegist(delegate {
                        // 遍历
        ...
        /// UpdateService
        /// </summary>
        public class UpdateService : SerialService
        {
            /// <summary>
            /// 定义事务服务
            /// </summary>
            ...
            public Serial Execute(Serial entity)
            {
                ...
                {
                    throw new Exception("MISApi.Services.CMS.Base.SerialService.UpdateService.Execute", ex);
                }
            }
            /// <summary>
            /// 默认的事务方法
        ...
        /// DeleteService
        /// </summary>
        public class DeleteService : SerialService
        {
            /// <summary>
            /// 定义事务服务
            /// </summary>
            ...
            public Serial Execute(Serial entity)
            {
                ...
                {
                    throw new Exception("MISApi.Services.CMS.Base.SerialService.DeleteService.Execute", ex);
                }
            }
            /// <summary>
            /// 默认的事务方法
            ...
            /// <returns></returns>
            public List<Serial> Execute(List<Serial> entitys)
            {
                try
                {
                    // 定义
        ...
        /// ColumnsMode Service
        /// </summary>
        public class ColumnsService : SerialService
        {
            /// <summary>
            /// 返回字段集
            /// </summary>
            /// <returns></returns>
            public Serial Single()
            {
                try
                {
                    return new Serial();
                ...
                {
                    throw new Exception("MISApi.Services.CMS.Base.SerialService.ColumnsService.Execute", ex);
                }
            }
        }
        ...
        /// 
        /// </summary>
        public class RowService : SerialService
        {
            /// <summary>
            /// 根据 id 查询
            /// </summary>
            ...
            /// <returns></returns>
            public Serial ById(int id, params BaseMode.Join[] joins)
            {
                    ...
                    try
                    {
                        return SQLEntityToSingle(
                            SQLQueryable(context, joins)
                                .Where(row => row.Serial.Id == id)
                                .SingleOrDefault()
                        );
                    }
                    catch (Exception ex)
                    {
                        throw new Exception("MISApi.Services.CMS.Base.SerialService.RowService.ById", ex);
                    }
                }
            }
        }
        ...
        public class RowsService : SerialService
        {
            ...
            /// <param name="joins">关联表</param>
            /// <returns></returns>
            public List<Serial> ByKeyWord(BaseMode.KeyWord keyWord, params BaseMode.Join[] joins)
            {
                try
                {
                ...
                {
                    throw new Exception("MISApi.Services.CMS.Base.SerialService.RowsService.ByKeyWord", ex);
                }
            }
            /// <summary>
            /// 
            ...
            /// <returns></returns>
            public List<Serial> Page(BaseMode.KeyWord keyWord, BaseMode.Join[] joins, BaseMode.Page page, BaseMode.Date[] dates, BaseMode.Sort[] sorts, BaseMode.Status status)
            {
                using (PandoraContext context = new PandoraContext())
                {
                    try
                    ...
                    {
                        throw new Exception("MISApi.Services.CMS.Base.SerialService.RowsService.Page", ex);
                    }
                }
            }
            /// <summary>
            ...
            public int PageCount(BaseMode.KeyWord keyWord, BaseMode.Join[] joins, BaseMode.Date[] dates, BaseMode.Sort[] sorts, BaseMode.Status status)
            {
                using (PandoraContext context = new PandoraContext())
                {
                    ...
                    {
                        throw new Exception("MISApi.Services.CMS.Base.SerialService.RowsService.PageCount", ex);
                    }
                }
            }
            /// <summary>
            ...
            public SummaryEntity PageSummary(BaseMode.KeyWord keyWord, BaseMode.Join[] joins, BaseMode.Date[] dates, BaseMode.Sort[] sorts, BaseMode.Status status)
            {
                using (PandoraContext context = new PandoraContext())
                {
                    ...
                    {
                        throw new Exception("MISApi.Services.CMS.Base.SerialService.RowsService.PageSummary", ex);
                    }
                }
            }
        }
        ...
        protected IQueryable<SQLEntity> KeyWordQueryable(IQueryable<SQLEntity> queryable, string keyWord)
        {
            try
            {
                ...
                if (ands.Length > 1)
                {
                    for (var i = 0; i < ands.Length; i++)
                    {
                        ...
                        queryable = queryable.Where(row =>
                                row.Serial.SerialNo.Contains(andKeyWord) ||
                                row.Serial.PayerName.Contains(andKeyWord) ||
                                row.Serial.PayerRealName.Contains(andKeyWord) ||
                                row.Serial.ReceiverName.Contains(andKeyWord) ||
                                row.Serial.ReceiverRealName.Contains(andKeyWord) ||
                                row.Serial.Remark.Contains(andKeyWord) ||
                                row.Serial.StatusName.Contains(andKeyWord)
                            );
                    }
                }
                ...
                {
                    queryable = queryable.Where(row =>
                            ors.Contains(row.Serial.SerialNo) ||
                            ors.Contains(row.Serial.PayerName) ||
                            ors.Contains(row.Serial.PayerRealName) ||
                            ors.Contains(row.Serial.ReceiverName) ||
                            ors.Contains(row.Serial.ReceiverRealName) ||
                            ors.Contains(row.Serial.Remark) ||
                            ors.Contains(row.Serial.StatusName)
                        );
                }
                else
                {
                    queryable = queryable.Where(row =>
                            row.Serial.SerialNo.Contains(keyWord) ||
                            row.Serial.PayerName.Contains(keyWord) ||
                            row.Serial.PayerRealName.Contains(keyWord) ||
                            row.Serial.ReceiverName.Contains(keyWord) ||
                            row.Serial.ReceiverRealName.Contains(keyWord) ||
                            row.Serial.Remark.Contains(keyWord) ||
                            row.Serial.StatusName.Contains(keyWord)
                        );
                }
                // 返回
                return queryable;
            ...
            {
                throw new Exception("MISApi.Services.CMS.Base.SerialService.KeyWordQueryable", ex);
            }
        }
        /// <summary>
        /// 关键字扩展
        ...
        protected IQueryable<SQLEntity> KeyWordExtQueryable(IQueryable<SQLEntity> queryable, string keyWordExt)
        {
                ...
                for (var i = 0; i < splits.Length; i++)
                {
                    try
                    {
                        ...
                        if (splits[i].ToLower().Contains("memberid"))
                        {
                            ...
                            queryable = queryable.Where(row => row.Serial.ReceiverId == memberId || row.Serial.PayerId == memberId);
                        }
            ...
            {
                throw new Exception("MISApi.Services.CMS.Base.SerialService.KeyWordExtQueryable", ex);
            }
        }
        /// <summary>
        /// 日期
        ...
        protected IQueryable<SQLEntity> DateQueryable(IQueryable<SQLEntity> queryable, params BaseMode.Date[] dates)
        {
            ...
            {
                throw new Exception("MISApi.Services.CMS.Base.SerialService.DateQueryable", ex);
            }
        }
        /// <summary>
        /// 状态
        ...
        protected IQueryable<SQLEntity> StatusQueryable(IQueryable<SQLEntity> queryable, BaseMode.Status status)
        {
            ...
            {
                throw new Exception("MISApi.Services.CMS.Base.SerialService.StatusQueryable", ex);
            }
        }
        /// <summary>
        /// 排序
        ...
        protected IQueryable<SQLEntity> SortQueryable(IQueryable<SQLEntity> queryable, params BaseMode.Sort[] sorts)
        {
            ...
            {
                throw new Exception("MISApi.Services.CMS.Base.SerialService.SortQueryable", ex);
            }
        }
        /// <summary>
        /// 分页
        ...
        protected IQueryable<SQLEntity> PageQueryable(IQueryable<SQLEntity> queryable, BaseMode.Page page)
        {
            ...
            {
                throw new Exception("MISApi.Services.CMS.Base.SerialService.PageQueryable", ex);
            }
        }
        /// <summary>
        /// SQLEntity转换
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        protected Serial SQLEntityToSingle(SQLEntity entity)
        {
            try
            {
                //判断搜索结果是否为空
            ...
            {
                throw new Exception("MISApi.Services.CMS.Base.SerialService.SQLEntityToSingle", ex);
            }
        }
        /// <summary>
        /// SQLEntity转换
        ...
        /// <returns></returns>
        protected List<Serial> SQLEntityToList(List<SQLEntity> list)
        {
            try
            {
            ...
            {
                throw new Exception("MISApi.Services.CMS.Base.SerialService.SQLEntityToList", ex);
            }
        }
